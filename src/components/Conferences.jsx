...
const Conferences = () => {
  ...
  function handleFilter(filterObject, searchObject) {
    setFilter({ ...filterObject, ...searchObject });
  }
  ...
  let searchedConferences = [];
  if (filter.criteria) {
    if (["city"].includes(filter.criteria)) {
      searchedConferences = filteredConferences.filter(conference => conference[filter.criteria]?.toLowerCase().includes(filter.searchValue?.toLowerCase()))
    } else{
      searchedConferences = filteredConferences.filter(conference => conference[filter.criteria]?.toLowerCase().includes(filter.searchValue?.toLowerCase()))
    }
  } else {
    searchedConferences = filteredConferences;
  }
  ...
  return (
    <div>
      ...
      {searchedConferences.length > 0 ? (
        <div className="container grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 justify-center p-4">
          {searchedConferences
